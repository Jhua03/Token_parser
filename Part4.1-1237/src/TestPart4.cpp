 ...
 *     Authors: Craig Scratchley and Mohammad Akbari
 *
...
{
	istringstream input("double d; d=6/4.5/3+4;");
	istringstream expectedOutput ("double d ; "
								  "d = 6 / 4.5 / 3 + 4 ;");
	check(input, expectedOutput);
}
...
{
	istringstream input ("double d; d = errno ? errno ? 5.5:2:3 ;");
	istringstream expectedOutput ("double d ; "
								  "d = errno ? errno ? 5.5 : 2 : 3 ;");
	check(input, expectedOutput);
}
...
{
	istringstream input ("double a; errno >>= a?a:2 ;");
	istringstream expectedOutput ("SyntaxError");
	check(input, expectedOutput);
}
...
{
   istringstream input ("double d; f = 5.. ;");
   istringstream expectedOutput ("SyntaxError");
   check(input, expectedOutput);
}
