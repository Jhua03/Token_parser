...
void check(std::string input, vector<tokenStruct> tokens)
{
	...
	for(unsigned int i{0}; i<min(tokens.size(),tokenObjects.size()); i++)
	{
		...
		if(type==tokens.at(i).tokenType) {
			if(type=="ensc251::int_literal<int>")
			{
				ensc251::numeric_literal<int> *intToken{dynamic_cast<ensc251::int_literal<int> *>(tokenObjects.at(i).get())};
			...
			else if(type=="ensc251::numeric_literal<double>")
			{
				ensc251::numeric_literal<double> *doubleToken{dynamic_cast<ensc251::numeric_literal<double> *>(tokenObjects.at(i).get())};
				BOOST_CHECK_EQUAL(doubleToken->getValue(), atof(tokens.at(i).tokenValue.c_str()) );
				BOOST_CHECK_EQUAL(doubleToken->getLineIndex(), tokens.at(i).lineIndex);
				BOOST_CHECK_EQUAL(doubleToken->getPosIndex(), tokens.at(i).posIndex);
			}
			else if(type=="ensc251::int_literal<char>")
			{
				ensc251::numeric_literal<char> *charToken{dynamic_cast<ensc251::int_literal<char> *>(tokenObjects.at(i).get())};
			...
			{
				ensc251::Token *strToken{dynamic_cast<ensc251::Token *>(tokenObjects.at(i).get())};
...
BOOST_AUTO_TEST_CASE( test2 ){
	...
	vector<tokenStruct> tokens   {{"char", 0, 0, "ensc251::type_spec"},
								  ...
								  {"'c'", 0, 9, "ensc251::int_literal<char>"},
								  {";", 0, 13, "ensc251::punctuator"}};
	check(input, tokens);
}

BOOST_AUTO_TEST_CASE( test3 ){
	std::string input{"double _dnumber; _dnumber += 10.5 % 5;"};
	vector<tokenStruct> tokens   {{"double", 0, 0, "ensc251::type_spec"},
								  {"_dnumber", 0, 7, "ensc251::numeric_id"},
								  {";", 0, 15, "ensc251::punctuator"},
								  {"_dnumber", 0, 17, "ensc251::numeric_id"},
								  {"+=", 0, 26, "ensc251::gen_assignment_operator"},
								  {"10.5", 0, 29, "ensc251::numeric_literal<double>"},
								  {"%" , 0, 34, "ensc251::mod_operator"},
								  {"5", 0, 36, "ensc251::int_literal<int>"},
								  {";", 0, 37, "ensc251::punctuator"}};
	check(input, tokens);
}
BOOST_AUTO_TEST_CASE( test4 ){
	...
	vector<tokenStruct> tokens   {{"char", 0, 0, "ensc251::type_spec"},
								  ...
								  {"10", 0, 19, "ensc251::int_literal<int>"},
								  {":", 0, 22, "ensc251::punctuator"},
								  {"20", 0, 24, "ensc251::int_literal<int>"},
...
BOOST_AUTO_TEST_CASE( test5 ){
	...
	vector<tokenStruct> tokens   {{"errno", 0, 0, "ensc251::int_id"},
								  ...
								  {"'2'", 0, 10, "ensc251::int_literal<char>"},
								  {">>", 0, 14, "ensc251::shift_operator"},
								  {"5", 0, 17, "ensc251::int_literal<int>"},
								  {";", 0, 19, "ensc251::punctuator"}};
	check(input, tokens);
}

BOOST_AUTO_TEST_CASE( test6 ){
	std::string input{"double _d; \n_d= 39E-2 - ( _d ++ ) ;"};
	vector<tokenStruct> tokens   {{"double", 0, 0, "ensc251::type_spec"},
								  {"_d", 0, 7, "ensc251::numeric_id"},
								  {";", 0, 9, "ensc251::punctuator"},
								  {"_d", 1, 0, "ensc251::numeric_id"},
								  {"=", 1, 2, "ensc251::gen_assignment_operator"},
								  {"39E-2", 1, 4, "ensc251::numeric_literal<double>"},
								  {"-", 1, 10, "ensc251::additive_operator"} ,
								  {"(", 1, 12, "ensc251::punctuator"} ,
								  {"_d", 1, 14, "ensc251::numeric_id"},
...
BOOST_AUTO_TEST_CASE( test7 ){
	...
	vector<tokenStruct> tokens   {{"char", 0, 0, "ensc251::type_spec"},
								  ...
								  {"\"Hello World\"", 0, 13, "ensc251::string_literal"},
								  {";", 0, 27, "ensc251::punctuator"}};
	check(input, tokens);
}
